...
const PublicStore: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFD] font-['Inter'] selection:bg-black selection:text-white">
      {/* Navigation Header */}
      <nav className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-700 px-6 py-6 flex items-center justify-between ${
        scrolled ? 'bg-white/90 backdrop-blur-2xl border-b border-slate-100 py-4 shadow-sm' : 'bg-transparent text-white'
      }`}>
        ...
        <div className="flex items-center gap-6">
          <div className="relative group hidden md:block">
            ...
            <input 
              ...
              className={`bg-transparent border-b pl-7 pr-4 py-1 text-sm outline-none transition-all w-40 focus:w-64 ${
                scrolled ? 'border-slate-200 text-black focus:border-black' : 'border-white/20 text-white focus:border-white'
              }`}
            />
          </div>
        ...
        <div className="max-w-7xl mx-auto px-6 overflow-x-auto no-scrollbar flex items-center gap-12 py-5">
          ...
          {CATEGORIES.map(cat => (
            <button 
              ...
              className={`text-[11px] font-black uppercase tracking-[0.2em] whitespace-nowrap transition-all relative py-2 ${
                activeCategory === cat ? 'text-black' : 'text-slate-400 hover:text-black'
              }`}
            >
              {cat}
              {activeCategory === cat && (
