...
const CreativeStudio: React.FC = () => {
  ...
  const handleGenerateClick = () => {
    ...
    if (scene) actions.generateCampaign(scene.prompt);
  };
            ...
            {generatedImages.map((img, idx) => (
              <div key={idx} className={`group relative rounded-[2.5rem] overflow-hidden bg-white shadow-2xl border-4 border-white animate-in zoom-in-95 duration-700 ${state.format === '9:16' ? 'aspect-[9/16]' : state.format === '16:9' ? 'aspect-video' : 'aspect-[3/4]'}`}>
                ...
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-4">
                   ...
                   <button onClick={() => {const link = document.createElement('a'); link.href = img; link.download = 'campaign_result.png'; link.click();}} className="w-48 py-3 bg-indigo-600 text-white rounded-xl text-[9px] font-black uppercase tracking-widest flex items-center justify-center gap-2 hover:scale-105 transition-transform"><Download size={14} /> Download HQ</button>
