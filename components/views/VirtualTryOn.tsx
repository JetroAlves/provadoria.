...
const VirtualTryOn: React.FC = () => {
  ...
  const handleGenerateOutfit = async () => {
    ...
    try {
      ...
      console.error(err);
      setError('Erro na geração. Tente menos peças ou outra foto.');
    } finally {
              ...
              filteredProducts.map(p => {
                ...
                return (
                  <button 
                    ...
                    className={`w-full flex items-center gap-3 p-2 rounded-2xl border-2 transition-all text-left group ${
                      isSelected 
